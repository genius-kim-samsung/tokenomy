...
여러 기기를 쓰고(예: 일반 업무는 윈도우, AI 개발·빌드는 우분투 데스크탑) 모니터 꺼둔
기기의 사용량을 다른 기기에서 보고 싶다면, **대시보드를 네트워크에 노출하지 말고 SSH
터널로** 당겨오면 된다. 앱은 여전히 `127.0.0.1` 에만 바인딩하고, `ssh -L` 이 양 끝단 모두
127.0.0.1 에 바인딩하므로 데이터는 종단간 로컬을 유지한다(자세한 배경은 ADR 0029).

```bash
# 클라이언트(예: 윈도우 노트북)에서 — 원격(예: 우분투)의 8765 를 로컬로 당겨온다
ssh -L 8765:127.0.0.1:8765 <원격-호스트>
# 그 뒤 클라이언트 브라우저에서 http://localhost:8765
```

- 포트는 보통 `8765` 지만 점유 시 `8766`~`8784` 로 밀린다 — 밀렸다면 원격의
  `<data>/runtime.json`(소스 실행은 `<repo>/data/runtime.json`)에서 실제 포트를 확인한다.
- 원격 앱은 켜져만 있으면 된다(창을 트레이로 숨겨도 서버는 계속 `127.0.0.1` 에서 서빙).
- 역방향(예: 우분투에서 윈도우 보기)도 대칭이다 — 윈도우에서 **OpenSSH Server**(기본
  꺼짐)를 켜면 같은 한 줄로 된다.
...
- Claude Code / Codex CLI / gemini-cli를 쓰던 계정 — 공식 사용량은 `~/.claude/.credentials.json`,
  `~/.codex/auth.json`, `~/.gemini/oauth_creds.json`을 읽어 자동으로 뜬다(없어도 로컬 로그 기반으로는 동작).
- 엔터프라이즈 Gemini만 추가로 `GOOGLE_CLOUD_PROJECT`(GCP 프로젝트) 환경변수가 필요하다 —
  단, **앱 메뉴 실행은 `~/.bashrc`를 못 읽으니** 아래 [문제 해결](#문제-해결)의 배치 위치를 따른다.
...
- **Gemini 공식 사용량만 안 뜸(claude·codex는 정상)** — 엔터프라이즈 Gemini는 GCP 프로젝트를
  `GOOGLE_CLOUD_PROJECT` 환경변수로 지정해야 한다. 그런데 **앱 메뉴(아이콘)로 실행하면 `~/.bashrc`를
  읽지 않아**, 셸에만 `export`해 둔 값이 앱에 전달되지 않는다(터미널에서 `echo $GOOGLE_CLOUD_PROJECT`는
  값이 보여도 앱 프로세스엔 없다). GUI·터미널이 모두 읽는 위치에 두고 **데스크톱 세션을 로그아웃/재로그인**한다:
  ```bash
  # 시스템 전역(권장, sudo 1회) — GUI·터미널·SSH 모두 반영
  echo "GOOGLE_CLOUD_PROJECT=<프로젝트값>" | sudo tee -a /etc/environment
  # 또는 sudo 없이 개인 계정만: echo 'export GOOGLE_CLOUD_PROJECT=<값>' >> ~/.profile
  ```
  재로그인 후 반영 확인:
  ```bash
  tr '\0' '\n' < /proc/"$(pgrep -f tokenomy.launcher | head -1)"/environ | grep GOOGLE_CLOUD_PROJECT
  ```
  `~/.bashrc`의 기존 줄은 그대로 둬도 무해하다(재로그인 후엔 새 터미널도 이 값을 가진다).
