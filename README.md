...
- **대시보드** — 이번 달 통합 전망(활성 AI를 USD 풀로 합쳐 월말 여유/부족 추정), 총지출(활성 AI 합산),
  추세 차트(한도선·예상선), 토큰 구성, 효율 코치, 프로젝트별 비용(Top 10), 복기용 최근 비싼 세션(Top 10).
...
- **미니 뷰** — 큰 창과 **배타 전환**되는 작은 글랜스 창(exe·네이티브 창 한정). 사이드바 "⊟ 미니뷰"로 전환.
...
공식 API를 단발 호출(≤3s, 백오프 없음)하고, Claude 월 한도·Codex 주간 크레딧 등 공식 버킷을 미러링한다.
