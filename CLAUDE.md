...
파싱해 월 예산 대비 번다운·프로젝트/세션별 비용·캐시 효율을 보여준다. 단발(single-shot)
데스크톱 앱(exe) 또는 소스로 실행. 전 과정 로컬, **대화 원문은 저장하지 않는다(토큰 메타만)**.
...
- **프라이버시 경계 — 깨지 말 것.** 파서는 토큰 usage 메타만 추출한다. content/프롬프트/대화 원문을
  DB·archive에 절대 넣지 않는다.
