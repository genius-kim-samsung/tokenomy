...
- **official_fetch.py** — 공식 사용량 라이브 취득(유일한 아웃바운드). **tracked_providers** 목록에 포함된 provider만 호출한다. 각 CLI의 로컬 OAuth 토큰을
  **읽기 전용**으로 사용해 공식 API를 단발 GET(≤3s/provider, 백오프 없음). 엔드포인트: Claude `https://api.anthropic.com/api/oauth/usage`, Codex `https://chatgpt.com/backend-api/wham/usage`. default-on + throttle(`min_interval_minutes` 기본 5). 401→auth_error, 그 외 실패→http_error,
