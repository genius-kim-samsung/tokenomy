...
import tokenomy.aggregate
from tokenomy.aggregate import (
    DayPoint, DaySessionRow,
    _trailing_business_days, _windowed_rows,
    by_day_session, by_dimension, by_model, by_project, by_session,
...
from tokenomy.official_aggregate import combined_forecast, official_view
...
# ── 기간별 사용량 카드 기반(ADR 0017) — 로컬 임의 구간 합 ──────────
...
def test_aggregate_has_no_import_of_official_aggregate():
    # 분할 불변식: aggregate(로컬 롤업)는 official_aggregate(공식)를 import하지 않는다.
    # module-level·함수-지역 어느 쪽이든 위반 — 소스 문자열로 둘 다 막는다.
    src = Path(tokenomy.aggregate.__file__).read_text(encoding="utf-8")
    assert "from tokenomy.official_aggregate" not in src
    assert "import tokenomy.official_aggregate" not in src
    assert "from tokenomy import official_aggregate" not in src
