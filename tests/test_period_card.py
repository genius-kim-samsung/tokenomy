...
구독/로컬=로컬 JSONL 달력 기간 합. 페이스 신호=이전 동일 구간(same-span) ▲▼%.
"""
...
def test_pace_up_down_flat_none():
    """페이스 = 현재 vs 이전 동일 구간 변화율. 이전 없음/0이면 None(비교 불가)."""
...
def test_period_card_local_mode_with_pace():
    """구독/로컬: 오늘/이번주/이번달=로컬 달력 합 + same-span 페이스, share에 꼬리표."""
    ...
    _msg(conn, "2026-06-09T01:00:00Z", 3.0)   # KST 6/09 10:00 (이번주·어제 same-span)
    _msg(conn, "2026-06-03T01:00:00Z", 2.0)   # KST 6/03 10:00 (이번달·지난주 same-span)
    _msg(conn, "2026-05-05T01:00:00Z", 4.0)   # KST 5/05 10:00 (지난달 same-span)
...
def test_period_card_official_today_pace():
    ...
    assert p["오늘"]["pace"] == {"dir": "up", "pct": 50}  # 9 vs 어제 6(16-10)
