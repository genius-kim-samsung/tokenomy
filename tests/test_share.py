...
def _seed_gemini_rate_window(conn, utils):
    """gemini rate_window 버킷들(USD 없음, 클래스 util%)을 NOW6 스냅샷으로 적재.

    utils=[(라벨, util)] 또는 [(라벨, util, resets_at)] — 3-튜플이면 만료 시각 지정(기본 None=만료 없음).
    """
    def _bucket(label, util, resets_at=None):
    ...
    insert_official_buckets(
        ...
        buckets=[_bucket(*t) for t in utils])
...
def test_share_context_gemini_excludes_expired_rate_window():
    """만료(resets_at 과거) rate_window 버킷은 카드처럼 공유 문구에서도 제외한다(codex P2)."""
    conn = connect(":memory:")
    _seed_day(conn, "claude", 9, 20.0)
    _seed_day(conn, "claude", 10, 30.0)
    expired = NOW6 - timedelta(days=2)   # OfficialBucket.resets_at은 datetime(문자열 아님, official_parser 계약)
    _seed_gemini_rate_window(conn, [("Pro", 12.0, expired), ("Flash", 3.0, None)])
    ctx = share_context(conn, {"tracked_providers": ["claude", "gemini"]}, NOW6)
    assert ctx is not None
    assert "Pro" not in ctx["text"]         # 만료 → 제외
    assert "Flash" in ctx["text"]           # 라이브 → 표시
