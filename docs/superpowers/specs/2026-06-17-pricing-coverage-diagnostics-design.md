...
- 메인테이너가 `pricing.json`/`pricing_overrides`를 정밀화하도록 유도한다(가장 actionable한 주체).
...
- 사용자 단가 입력 UI — 안 함(`overrides`는 기존 config 경로 유지).
...
│    있으니 pricing.json에 별도 항목 추가를 검토하세요.    │
...
- **변경**: `aggregate.py`(신규 함수+overview 경고), `web/views.py`(카드 컨텍스트),
  `web/templates/settings.html`(카드), `cli.py`(report 한 줄), 테스트.
- **비변경**: `pricing.json` 스키마, `compute_cost`/`find_rate` 시그니처, `db.py` 스키마, 적재 파이프라인.
  진단은 **읽기 전용 집계**다 — 단가 계산·DB 적재는 손대지 않는다.
